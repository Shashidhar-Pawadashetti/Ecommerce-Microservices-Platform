...
Note: this directory IS already a git repository (initialized during project setup; HEAD contains planning-only commits — no product code or `.gitattributes` yet). Plan 01-01's one-time `git add --renormalize .` is therefore required (the fresh-repo exemption does NOT apply), and `.gitattributes` still lands as the first product commit before any scaffold file.
