...
**Core Value:** A complete end-to-end purchase journey — signup → browse products → add to cart → checkout → payment → order status update → notification — working across all services through the gateway, verified by `docker compose up` plus a scripted smoke test.
